...
test_socket_params( "127.0.0.1:6000",
...
test_socket_params( "inet:127.0.0.1:6000",
                    Milter::SMTPAuth::SocketParams::INET,
                    sub {
                        my ( $param ) = @_;
...
test_socket_params( "inet:localhost.localdomains:6000",
                    Milter::SMTPAuth::SocketParams::INET,
                    sub {
                        my ( $param ) = @_;
                        is( $param->address(),
                            "localhost.localdomains",
                            "inet:localhost.localdomains:6000 address is localhost.localdomains" );
                        is( $param->port(),
                            6000,
                            "inet:localhost.localdomains:6000 port is port"  );
                    });

test_socket_params( "inet6:::1:6000",
...
test_socket_params( "unix:/var/run/filter.sock",
                    Milter::SMTPAuth::SocketParams::UNIX,
                    sub {
                        my ( $param ) = @_;
                        is( $param->address(),
                            "/var/run/filter.sock",
                            "unix:/var/run/filter.sock address is /var/run/filter.sock" );
                    });

test_socket_params( "/var/run/filter.sock",
                    Milter::SMTPAuth::SocketParams::UNIX,
                    sub {
                        my ( $param ) = @_;
                        is( $param->address(),
                            "/var/run/filter.sock",
                            "/var/run/filter.sock address is /var/run/filter.sock" );
                    });
